import WebVRManager from './webvr-manager.js'
import WebVRButtonDOM from './webvr-button-dom.js'
        ...
        this.webvrmanager.getPresentableDevice()
            .then((hmd) => {
                ...
                if(hmd.capabilities.canPresent){
                    this.setState(WebVRUIState.READY_TO_PRESENT)
                } else {
                    this.setState(WebVRUIState.ERROR_BROWSER_NOT_SUPPORTED)
                }
            })
            .catch((e) => {
                if(e.name == 'NO_DISPLAYS'){
                    this.setState(WebVRUIState.ERROR_NO_PRESENTABLE_DISPLAYS)
